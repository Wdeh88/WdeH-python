...
try:
   ...
   prints(nel(f"{P2}لاتوفر اتصال بالانترنت حاول مره اخرى",width=80,padding=(0,2),style=f"{color_panel}"));exit()
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} login good✅!!!!{x} ');time.sleep(1)
		exit()
...
def menu(_name_,_id_):
	...
	print(50*"\033[0;97m-")
	print('\n')
	print(f'{asu} من اصدقاء [1]')
	print(f'{asu} من ملف [2]')
	print(f'{asu} تسجيل خروج [3]')
	wdeh = input(f'اخـتـار >> ')
	if wdeh in ['1']:
		ksmk()
	elif wdeh in ['2']:
		file()
...
def file():
	try:
			print('')			
			fileX = input ('\x1b[1;93mFILE NAME | اسم الملف ')
			print('\n')
...
def ksmk():
	...
	try:
		print('\n')
		jum = int(input('\x1b[1;91m➺ Number ID | عـدد الـايـديـات : '))
		print('')
	except ValueError:
		print(' ')
		exit()
	...
	for met in range(jum):
		yz+=1
		kl = input('\x1b[38;5;216m➺ ID  '+str(yz)+' : ')
		uid.append(kl)
	for user in uid:
	    try:
	...
	try:
		print('')
		print(f'\033[1;32mTOTAL | ID ➜ {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print('')
